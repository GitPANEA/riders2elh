...
### Bug: `CK_BATCH_CONTATORI` bloccava i batch grandi in `IN_CORSO` per sempre (24 agosto 2026)

Occorso durante un test di carico su `docs/v2/` (~40.000 anagrafiche): il batch restava bloccato in `IN_CORSO`, con nel journal `ORA-02290: check constraint (CK_BATCH_CONTATORI) violated` seguito da un secondo, identico fallimento con il messaggio *"impossibile marcare il batch come ERRORE_TECNICO, resta IN_CORSO"` — il caso limite già previsto dal commento su `elaboraAsync` (vedi sopra, "Esecuzione asincrona dei caricamenti"), qui effettivamente realizzato.

**Causa radice.** `BatchCaricamentoRepository.aggiornaProgresso` (rimosso, vedi sotto) scriveva `UPDATE T_BATCH_CARICAMENTO SET NUM_RECORD_OK = ?, NUM_RECORD_KO = ? WHERE ID_BATCH = ?` ogni `intervallo-progresso` record (1000, default), **senza** toccare `NUM_RECORD_TOTALI` — già valorizzato da `avviaElaborazione` all'inizio dell'elaborazione. Il `CK_BATCH_CONTATORI` (`05-ddl-check-coerenza.sql`) impone che i tre contatori siano *tutti* `NULL` oppure che `NUM_RECORD_TOTALI = NUM_RECORD_OK + NUM_RECORD_KO` **in ogni istante**: a metà elaborazione questo non è mai vero per costruzione (es. `TOTALI=39850, OK=1000, KO=0`), quindi il primo aggiornamento di progresso su un batch sopra soglia fa fallire l'`UPDATE`. Il fallback che tenta `chiudiBatch(..., ERRORE_TECNICO, ...)` scrive invece tutti e tre insieme, ma nel caso osservato ha fallito a sua volta con lo stesso `ORA-02290`, lasciando il batch bloccato senza via d'uscita. Un batch sotto la soglia (es. un test da 150 record) non manifesta mai il problema, perché `aggiornaProgresso` semplicemente non viene mai invocato in corsa — motivo per cui il bug è sfuggito ai primi test.

**Fix (non un compromesso: scelta esplicita del cliente).** Interpellato sul punto, il cliente ha indicato che il progresso in corsa non serve — l'utente deve solo sapere che il batch è `IN_CORSO`, l'esito con i contatori reali arriva a fine elaborazione via polling. È stato quindi **rimosso l'aggiornamento incrementale dei contatori**, non allentato il `CHECK`: `aggiornaProgresso` è stato eliminato da `BatchCaricamentoRepository`, così come la sua chiamata nel ciclo `for` dei tre service (`AnagraficaService`, `VoceService`, `MovimentazioneService`) e il parametro/campo `intervalloProgresso` iniettato via `@Value`; la property `riders2eLH.batch.intervallo-progresso` è stata rimossa da `application.yml`. Ora `NUM_RECORD_OK`/`NUM_RECORD_KO` restano `NULL` per tutta la durata di `IN_CORSO` (ramo `NULL/NULL/NULL` del `CHECK`, sempre rispettato) e vengono scritti tutti insieme, coerenti, solo da `chiudiBatch` a fine elaborazione — sia in caso di successo sia nel fallback `ERRORE_TECNICO`.

Se in futuro dovesse servire di nuovo un progresso in tempo reale (es. per batch che durano molti minuti), la strada corretta non è ripristinare `aggiornaProgresso` così com'era, ma allentare `CK_BATCH_CONTATORI` per ammettere anche lo stato intermedio (`OK + KO <= TOTALI`, non necessariamente l'uguaglianza) mantenendo l'uguaglianza stretta solo a batch chiuso.

### Bug: formato di `MESE_RIFERIMENTO` invertito tra Java e Oracle (27 agosto 2026)

Occorso durante un test di carico sulle movimentazioni: un record con `mese_riferimento: "2026-06"` — valore corretto secondo lo schema — veniva respinto con `"mese_riferimento non valido, atteso mm-yyyy: 2026-06"`.

**Causa radice.** Il DDL (`T_MOVIMENTAZIONE_VOCE_ST.MESE_RIFERIMENTO`, commento `-- formato 'YYYY-MM'`) e il `CK_MOVVOCE_MESE_RIFERIMENTO` (`05-ddl-check-coerenza.sql`, `REGEXP_LIKE(MESE_RIFERIMENTO, '^[0-9]{4}-(0[1-9]|1[0-2])$')`) vogliono `yyyy-mm` fin dalla loro introduzione il 20 agosto 2026. Il pattern Java in `ValidatoreFormato` (`MESE_RIFERIMENTO`) validava invece `mm-yyyy` — l'ordine invertito — fin dalla sua stessa introduzione: non un cambio di requisito, un disallineamento applicazione/DB mai emerso finché non è arrivato un valore realistico in `yyyy-mm` (i primi test avevano probabilmente usato valori che non discriminavano i due ordini, o erano finiti KO per altri motivi prima di arrivare a questo controllo).

**Fix.** Pattern corretto in `^\d{4}-(0[1-9]|1[0-2])$` in `ValidatoreFormato.java`, messaggio di violazione aggiornato di conseguenza. Il campo è trattato come stringa grezza ovunque nel codice (mai parsato in un tipo data con un ordine assunto), quindi il fix è isolato a quel pattern — nessun altro punto (repository, DTO, controller) presuppone un ordine mm/yyyy specifico.

### Bug: `@Valid List<...>` sulle POST di caricamento risponde `500` invece di `400`, con Spring Framework 6.1+ (31 agosto 2026)

Occorso durante un caricamento reale del cliente (non un dataset di test): l'API rispondeva `500 Errore interno del server`, mentre lo stesso identico file, caricato manualmente in dev, veniva elaborato senza problemi — quindi non un difetto nel payload né nel codice di elaborazione, ma qualcosa che dipendeva da un dettaglio della richiesta stessa.

**Causa radice.** Le tre POST di caricamento (`/anagrafiche`, `/movimentazioni`, `/voci`) validano il body con `@RequestBody @Valid List<...Dto>` — l'argomento annotato con `@Valid` è una **collezione grezza**, non un singolo oggetto. Da Spring Framework 6.1 (in uso da questo progetto via Spring Boot 3.3.4, confermato `spring-web-6.1.13` nello stack trace) questo caso non passa più per la storica `MethodArgumentNotValidException`, ma per un tipo nuovo introdotto insieme al meccanismo di "method validation": `HandlerMethodValidationException`. `GlobalExceptionHandler` gestiva solo la prima; la seconda cadeva nella rete di sicurezza generica `Exception`→500 — lo stesso travestimento già visto altre volte in questo progetto (Content-Type→403 fuorviante, tipo di parametro→500), qui su un percorso però mai esercitato dai test precedenti: i dataset sintetici di `docs/v2/` passavano sempre la validazione, quindi l'eccezione non si era mai manifestata prima di un payload reale del cliente con un record davvero non conforme.

**Perché lo stesso file "funzionava" a chi lo caricava e non al cliente**: `HandlerMethodValidationException` scatta solo se **almeno un elemento della lista** viola un vincolo Bean Validation (`@NotBlank`, `@NotNull`, ecc. sui DTO in `dto/`). Un file che sembra identico può comunque differire in un singolo record, o lo stesso file può essere inviato con encoding/serializzazione diversi da client diversi: la causa va sempre isolata dal messaggio d'errore, non per confronto visivo dei file.

**Fix, in due passi.** Primo: aggiunto `@ExceptionHandler(HandlerMethodValidationException.class)` in `GlobalExceptionHandler`, così la risposta diventa correttamente `400` (era già lo status che Spring stesso attribuiva internamente all'eccezione — visibile in `e.getMessage()`, `"400 BAD_REQUEST ..."` — solo instradato male dal codice applicativo). Secondo, perché il primo passo da solo non bastava: `e.getMessage()` su questo tipo vale sempre e solo il letterale `"400 BAD_REQUEST \"Validation failure\""`, senza nominare campo o vincolo — inutile per il cliente quanto un 500 generico. Il dettaglio reale sta in `e.getAllValidationResults()` (`List<ParameterValidationResult>`, package `org.springframework.validation.method`, **non** `org.springframework.web.method.annotation` nonostante il nome faccia pensare al contrario): ogni risultato espone `getResolvableErrors()` (il messaggio del vincolo violato) e `getContainerIndex()` (l'indice dell'elemento nella lista, quando il parametro è una collezione — qui coincide con la posizione del record nel payload originale). Il messaggio finale è ora del tipo `elemento 47: must not be blank`, che nomina la posizione del record malformato senza dover cercare `T_BATCH_CARICAMENTO_ERRORE` (che qui non è nemmeno coinvolta: la validazione fallisce **prima** che il service crei il batch, l'intera richiesta è respinta con 400, non isolata per record come nel flusso normale per-record già documentato sopra).
